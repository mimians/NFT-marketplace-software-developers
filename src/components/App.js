...
import SoftwareDevelopers from "../abis/SoftwareDevelopers.json";

import FormAndPreview from "../components/FormAndPreview/FormAndPreview";
import AllSoftwareDevelopers from "./AllSoftwareDevelopers/AllSoftwareDevelopers";
import AccountDetails from "./AccountDetails/AccountDetails";
import ContractNotDeployed from "./ContractNotDeployed/ContractNotDeployed";
import ConnectToMetamask from "./ConnectMetamask/ConnectToMetamask";
import Loading from "./Loading/Loading";
import Navbar from "./Navbar/Navbar";
import MySoftwareDevelopers from "./MySoftwareDevelopers/MySoftwareDevelopers";
...
class App extends Component {
  ...
  setMintBtnTimer = () => {
    ...
    if (mintBtn !== undefined && mintBtn !== null) {
      ...
      this.state.lastMintTime === undefined || this.state.lastMintTime === null
        ? (mintBtn.innerHTML = "Mint My Software Developer")
  ...
  loadBlockchainData = async () => {
    ...
    if (accounts.length === 0) {
      ...
      const networkData = SoftwareDevelopers.networks[networkId];
      if (networkData) {
        this.setState({ loading: true });
        const cryptoBoysContract = web3.eth.Contract(
          SoftwareDevelopers.abi,
          networkData.address
        );
  ...
  setMetaData = async () => {
    if (this.state.cryptoBoys.length !== 0) {
      this.state.cryptoBoys.map(async (softwaredeveloper) => {
        const result = await fetch(softwaredeveloper.tokenURI);
        const metaData = await result.json();
        this.setState({
          cryptoBoys: this.state.cryptoBoys.map((softwaredeveloper) =>
            softwaredeveloper.tokenId.toNumber() === Number(metaData.tokenId)
              ? {
                  ...softwaredeveloper,
                  metaData,
                }
  ...
  mintMyNFT = async (colors, name, tokenPrice) => {
    ...
    if (colorsUsed.length === 0 && !nameIsUsed) {
      ...
      this.state.cryptoBoysContract.methods
        .mintSoftwareDeveloper(name, tokenURI, price, colorsArray)
  ...
  buySoftwareDeveloper = (tokenId, price) => {
    this.setState({ loading: true });
    this.state.cryptoBoysContract.methods
      .buyToken(tokenId)
          ...
          <>
            <HashRouter basename="/">
              ...
              <Route
                path="/marketplace"
                render={() => (
                  <AllSoftwareDevelopers
                    accountAddress={this.state.accountAddress}
                    cryptoBoys={this.state.cryptoBoys}
                    totalTokensMinted={this.state.totalTokensMinted}
                    changeTokenPrice={this.changeTokenPrice}
                    toggleForSale={this.toggleForSale}
                    buySoftwareDeveloper={this.buySoftwareDeveloper}
                  />
                )}
              />
              <Route
