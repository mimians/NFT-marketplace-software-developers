...
const SoftwareDevelopers = artifacts.require("./SoftwareDevelopers.sol");

require("chai")
  .use(require("chai-as-promised"))
  .should();

contract("Software Developers", async (accounts) => {
  let cryptoBoys, result, cryptoBoyCount;

  before(async () => {
    cryptoBoys = await SoftwareDevelopers.deployed();
  });
  ...
  describe("Deployment", async () => {
    ...
    it("has a name", async () => {
      ...
      assert.equal(name, "Software Developers Collection");
    });
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      ...
      result = await cryptoBoys.mintSoftwareDeveloper(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      softwaredeveloper = await cryptoBoys.allSoftwareDevelopers(1, {
        from: accounts[0],
      });
      assert.equal(softwaredeveloper.tokenId.toNumber(), 1);
      assert.equal(softwaredeveloper.tokenName, "myCBNFT");
      assert.equal(
        softwaredeveloper.tokenURI,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
      assert.equal(softwaredeveloper.mintedBy, accounts[0]);
      assert.equal(softwaredeveloper.currentOwner, accounts[0]);
      assert.equal(
        softwaredeveloper.previousOwner,
        0x0000000000000000000000000000000000000000
      );
      assert.equal(web3.utils.fromWei(softwaredeveloper.price, "ether"), 1);
      assert.equal(softwaredeveloper.numberOfTransfers.toNumber(), 0);
      assert.equal(softwaredeveloper.forSale, true);
      ...
      await cryptoBoys.mintSoftwareDeveloper(
        "myCBNFT2",
    ...
    it("allows users to buy token for specified ethers", async () => {
      ...
      let cryptoBoy;
      cryptoBoy = await cryptoBoys.allSoftwareDevelopers(1, {
        from: accounts[0],
      });
    ...
    it("allows users to change token price", async () => {
      let cryptoBoyPrice;
      cryptoBoyPrice = await cryptoBoys.allSoftwareDevelopers(1, {
        from: accounts[0],
      });
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      assert.equal(softwaredeveloper.forSale, false);
